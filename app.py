...
def chemicalInfo(num):
    ...
    # return chemInfo['Record']['Section']
    return chemInfo.json()['Record']
    ...
    with open("D:\Ardhi\Ecoinvent\cut-off-system-model\Cut-off Cumulative LCIA v3.9.csv", 'r') as file:
       ...
       for row in csvreader:
           ...
           if(dis>maxdis):
            maxdis=dis
            max_sim_row=row
       return jsonify({'data': max_sim_row,'msf score':maxdis})  
...
@app.route('/readanduploadcsv', methods=['POST'])
def uploadandreadcsv():
    ...
    #             filename = file.filename
         with open('inputFile1.csv', 'r') as file1:
                csvreader = csv.reader(file1)
                for row in csvreader: 
                     url='http://127.0.0.1:5000/addlcia/'+row[0]+'/'+'inputFile1.csv'
                     response=requests.get(url)
                     print (response)
